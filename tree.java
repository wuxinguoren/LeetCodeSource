...
public class tree {
	...
	public static void main(String args[]) {
		...
		int[] b = { 1, 2, 3 };
		int[] c = { 3, 2, 1 };
		System.out.print(a.countNodes(root));
    ...
    public int countNodes(TreeNode root) {
        ...
        k = countFinalNodes(root,(int)Math.pow(2, h),h);
    ...
    public int countFinalNodes(TreeNode root, int k ,int h){
    	if(h==1){
            	if(root.right!=null){
            		return 2;
            	}
    	...
    	if(leftnode.right != null){
    		cur += countFinalNodes(root.right,cur,h-1);
    	}else{
    		if(leftnode.left != null){
    			return cur-1;
    		}else{
    			cur = countFinalNodes(root.left,cur,h-1);
    		}
    	}
